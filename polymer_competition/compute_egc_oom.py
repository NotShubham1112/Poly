...
for fold in range(5):
    ckpt = torch.load(f"outputs/gin/egc/checkpoints/gin_gin_fold{fold}_best.pt",
    ...
    print(f"  Fold {fold}: R2={r2_score(y_tr[va_idx], va_preds):.4f}")

print(f"GIN OOF R2: {r2_score(y_tr, all_gin_preds):.4f}")
...
for fold in range(5):
    ...
    print(f"  Fold {fold}: R2={r2_score(y_tr[vi], all_xgb_preds[vi]):.4f}")
...
for w in [0.3, 0.5, 0.7]:
    ...
    print(f"Blend w(xgb)={w}: R2={r2_score(y_tr, bl):.4f}")
...
# Save OOF files
pickle.dump({"val_idx": range(n_train), "pred": all_gin_preds, "y": y_tr,
             "model_type": "gin", "target": "egc"},
            open("outputs/gin/egc/oof_egc_gin.pkl", "wb"))
pickle.dump({"val_idx": range(n_train), "pred": all_xgb_preds, "y": y_tr,
             "model_type": "xgb", "target": "egc"},
            open("outputs/gin_xgb/egc/oof_egc_xgb.pkl", "wb"))
print("Saved OOF files")

# Test predictions
print("\n=== Generating test predictions ===")
te = pd.read_csv(data_dir / "test.csv")
tmask_te = te["target_type"].values == "egc"
te_smiles = te["smiles"].values[tmask_te]
te_ids = te["id"].values[tmask_te]
...
# GIN test predictions (average across 5 folds)
te_gin_preds = np.zeros(len(te_graphs), dtype=np.float32)
for fold in range(5):
    ckpt = torch.load(f"outputs/gin/egc/checkpoints/gin_gin_fold{fold}_best.pt",
                      map_location="cpu", weights_only=False)
    ms = ckpt["model_state"]
    ...
    with torch.no_grad():
        for batch in te_loader:
            p = model(batch)
            preds.append(p.numpy())
    te_gin_preds += np.concatenate(preds)
te_gin_preds /= 5

# XGB test predictions (average across 5 folds)
X_te = pd.read_parquet(data_dir / "processed/features_test.parquet")
X_te_arr = X_te[fc].values.astype(np.float32)[tmask_te]
X_te_s = scaler.transform(X_te_arr)

te_xgb_preds = np.zeros(len(te_graphs), dtype=np.float32)
for fold in range(5):
    ti, _ = splits[fold]["train"], splits[fold]["val"]
    m = xgb.XGBRegressor(**xgbp, early_stopping_rounds=50)
    m.fit(Xs[ti], y_tr[ti], eval_set=[(Xs[ti], y_tr[ti])], verbose=False)
    te_xgb_preds += m.predict(X_te_s)
te_xgb_preds /= 5

# Blend predictions
w_opt = res.x
blend_preds = w_opt * te_xgb_preds + (1-w_opt) * te_gin_preds

# Save
sub = pd.DataFrame({"id": te_ids, "target": blend_preds})
sub.to_csv("outputs/gin_xgb/egc/submission_blend_egc.csv", index=False)
print(f"Saved submission -> outputs/gin_xgb/egc/submission_blend_egc.csv")
print(f"Shapes: GIN={te_gin_preds.shape}, XGB={te_xgb_preds.shape}, Blend={blend_preds.shape}")
